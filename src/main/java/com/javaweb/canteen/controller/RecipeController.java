...
public class RecipeController {
    ...
    public R<Page<Recipe>> getAllRecipe(int page, int limit,
        ...
        queryWrapper.like(StrUtil.isNotEmpty(name), Recipe::getName, name)
                .and(w -> w.eq(Recipe::getDeleted, 0).or().isNull(Recipe::getDeleted))
    ...
    public R<String> delete(@PathVariable Long recipeId){
        ...
        if (recipeId != null) {
            ...
            updateWrapper.eq(Recipe::getRecipeId, recipeId).set(Recipe::getDeleted, 1);
            res = recipeService.update(null, updateWrapper);
        }
        if (res) {
    ...
    public R<String> deleteBatchRecipeIds(@PathVariable String recipeIds){
        ...
        for (String id : ids) {
            ...
            LambdaUpdateWrapper<Recipe> updateWrapper = new LambdaUpdateWrapper<>();
            updateWrapper.eq(Recipe::getRecipeId, recipeId).set(Recipe::getDeleted, 1);
            boolean flag = recipeService.update(null, updateWrapper);
            res = res && flag;
        }
        if (res) {
